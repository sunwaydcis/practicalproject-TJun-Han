...
class PersonEditDialogController ():
  ...
  def handleOk(action :ActionEvent): Unit = {
    ...
    if (isInputValid())
      ...
      //so parseLocalDate will change to local date, but local date is not one of the member of optiom
      // so need to use getOrELse, if true will come to get means return local date object

      okClicked = true
      dialogStage.close()
  }

  @FXML // it mention this action collaborate to fxml, only the function that need to bind to dxml
  ...
  def isInputValid() : Boolean =
    ...
    if (nullChecking(postalCodeField.text.value)) then
      ...
      try
        postalCodeField.getText().toInt //postalcodefield is non java fx object, so need call getText, 
        // to retrive the value inside the postalCodeField and change to String type
      catch
